...
describe 'testing GroupImage with group_caption' do
  before do
    ...
    @images_folder = "#{ENV["HOME"]}/test_data/group_image"
...
describe 'testing GroupImage creation' do
  before do
    # @container = Container.new(:width=>600, :height=>800, :layout_space=>20, :layout_direction=>"horizontal") do
    # @image_items = %w[1.jpg 2.jpg]
    @pdf_path = "#{ENV["HOME"]}/test_data/yearbook/sample_class/B/output.pdf"
    @images_folder = "#{ENV["HOME"]}/test_data/yearbook/sample_class/B/output.pdf"
    @images_folder = "#{ENV["HOME"]}/test_data/yearbook/sample_class/B"
    @image_items = Dir.glob("#{@images_folder}/*.jpg").map{|f| File.basename(f)}
    @image_item_captions = @image_items.map{|f| File.basename(f, ".jpg").unicode_normalize}
