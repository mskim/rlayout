...
module RLayout
  ...
  class RCoverPage < Container
    ...
    def initialize(options={}, &block)
      ...
      super
      @heading_height_type  = options[:heading_height_type] || "full"
    ...
    def set_content(options)
      ...
      if options[:logo]
        # text_alignment = options[:text_alignment] || 'center'
        # TODO
        # t = @logo_object = logo(options[:logo])
        # @logo_object.y = @y_position
        # @y_position += t.height
      end
    ...
    def background(image_path, options={})
      options[:local_image] = image_path
      RLayout::Image.new(atts).new(atts)
    end

    def logo(image_path, options={})
      atts = {}
      atts[:parent] = self
      atts[:local_image] = image_path
      atts[:x] = @width/2 - 25
      atts[:width] = 50
      atts[:height] = 50
      # RLayout::Image.new(atts)
    end
  end
