    ...
    class Pdf < Builder
      ...
      def build!(options={})
        ...
        if manifest.markdown?
          ...
          RLayout::Builders::Html.new.build!
          opts = options.merge({ source: :polytex, origin: :markdown})
          self.manifest = RLayout::BookManifest.new(opts)
        end
        ...
        polytex_filenames.each do |filename|
          ...
          if filename == book_filename
            latex.gsub!(/\\include{(.*?)}/) do
              "\\include{#{RLayout::Utils.tmpify(manifest, $1)}.tmp}"
            end
          end
          File.write(RLayout::Utils.tmpify(manifest, filename), latex)
        end
        write_pygments_file(:latex, RLayout::Directories::STYLES)
        copy_polytexnic_sty
      ...
      private
        ...
        def build_pdf(book_filename, options={})
          tmp_filename = RLayout::Utils.tmpify(manifest, book_filename)
        ...
        def keep_tmp_files?(options)
          options[:once] || options[:'find-overfull'] || RLayout.test?
        end

        # Copies the style file to ensure it's always fresh.
        def copy_polytexnic_sty
          softcover_sty  = File.join(RLayout::Directories::STYLES,
                                     'softcover.sty')
