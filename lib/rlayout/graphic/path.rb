module RLayout
  class Path < Graphic
    ...
    def parse
      ...
      data = @d.gsub(/#{OUTSIDE_SPACE_REGEXP}$/, '')
    ...
    def parse_path_command(command, values)
      ...
      when 'M' # moveto
        ...
        @path.moveToPoint(NSPointMake(x,y))
      ...
      when 'H' # horizontal lineto
        while values.any?
          ...
          @path.lineToPoint(NSPointMake(x,@last_point.last))
      ...
      when 'V' # vertical lineto
        while values.any?
          ...
          @path.lineToPoint(NSPointMake(@last_point.first,y))
      ...
      when 'C' # curveto
        while values.any?
          ...
          @path.curveTo(NSPointMake(x,y), NSPointMake(x1,y1), NSPointMake(x2,y2))
