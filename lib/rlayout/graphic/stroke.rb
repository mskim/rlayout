...
module RLayout
  ...
  class Graphic
    def init_stroke(options)
      ...
      @stroke[:color]     = options[:line_color] if options[:line_color]
      @stroke[:thickness] = options[:line_width] if options[:line_width]
      @stroke[:dash]      = options[:dash] if options[:dash]
      @stroke[:line_cap]  = options[:line_cap] if options[:line_cap]
      @stroke[:type]      = options[:type] if options[:type]
    end
