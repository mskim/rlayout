    ...
    class Html < Builder
      ...
      def build(options = {})
        ...
        if manifest.markdown?
          ...
          self.manifest = RLayout::BookManifest.new(source: :polytex,
                                                      verify_paths: false,
                                                      origin: :markdown)
        end
        ...
        if manifest.polytex?
          ...
          @title = basename
          @mathjax = RLayout::Mathjax::config(chapter_number: false)
          @src     = RLayout::Mathjax::AMS_SVG
      ...
      def polytex(chapter, markdown)
        ...
        p = Polytexnic::Pipeline.new(markdown,
                                     source: :markdown,
                                     custom_commands: RLayout.custom_styles,
      ...
      def write_complete_file(chapter, erb_file, n)
        ...
        File.open(html_filename, 'w') do |f|
          ...
          @mathjax = RLayout::Mathjax::config(chapter_number: n)
          @src     = RLayout::Mathjax::AMS_SVG
          file_content = ERB.new(erb_file).result(binding)
          f.write(file_content)
        end
