module RLayout
  class Builder
    ...
    def initialize
      @manifest = RLayout::BookManifest.new(verify_paths: true,
                                              source: source)
      @built_files = []
    ...
    private
      ...
      def ensure_style_file_locations
        styles_dir = RLayout::Directories::STYLES
        mkdir styles_dir
        fix_custom_include
        files = Dir.glob('*.sty')
      ...
      def fix_custom_include
        ...
        if File.exist?(first_chapter)
          ...
          text.gsub!('<<(custom.sty',
                     "<<(#{RLayout::Directories::STYLES}/custom.sty" )
      ...
      def styles_dir
        File.join(Dir.pwd, RLayout::Directories::STYLES)
      end
